...
describe("ScalarExprTreeBuilder", function () {
  beforeEach(function () {
    ...
    this.schema = this.schema.addTable({
      id: "t1",
      name: "T1",
      contents: [{ id: "c1", name: { en: "C1" }, type: "text" }]
    })

    return (this.schema = this.schema.addTable({
      id: "t2",
      name: "T2",
      label: "c1",
      contents: [{ id: "c1", name: { en: "C1" }, type: "text" }]
  ...
  it("returns columns", function () {
    ...
    return assert(
      _.isEqual(nodes[0].value, { table: "t1", joins: [], expr: { type: "field", table: "t1", column: "c1" } }),
      JSON.stringify(nodes[0].value)
    )
  })
  ...
  it("returns count expr as first if includeAggr is true", function () {
    ...
    return assert.deepEqual(
      nodes[0].value,
      { table: "t1", joins: [], expr: { type: "op", op: "count", table: "t1", exprs: [] } },
      JSON.stringify(nodes[0].value)
    )
  })
        ...
        tables: [
          {
            ...
            contents: [
              { id: "c1", name: { en: "c1" }, type: "text" },
              {
                ...
                name: { en: "c2" },
                type: "section",
                contents: [{ id: "c3", name: { en: "c3" }, type: "text" }]
              }
            ]
          }
        ]
                ...
                contents: [
                  {
                    ...
                    name: { en: "c3" },
                    type: "section",
                    contents: [{ id: "c4", name: { en: "c4" }, type: "text" }]
                  }
                ]
              }
            ]
  ...
  it("follows joins", function () {
    ...
    return assert(
      _.isEqual(subnode.children()[0].value, {
        table: "t1",
        joins: ["c2"],
        expr: { type: "field", table: "t2", column: "c1" }
      }),
      JSON.stringify(subnode)
    )
  })
  ...
  return describe("limits type", function () {
    ...
    return it("includes types formed by aggregation", function () {
      ...
      return assert.deepEqual(
        nodes[0].value.expr,
        { type: "op", op: "count", table: "t2", exprs: [] },
        JSON.stringify(nodes[0].value.expr)
      )
    })
  })
})
