...
const JoinEditorComponent = (props) => {
    ...
    const isStandard1toN = join &&
        inverse &&
        fromTable &&
        inverse.type == "id" &&
        !inverse.jsonql &&
        inverse.idTable == fromTableId &&
        join.toColumn == inverse.id &&
        join.fromColumn == fromTable.primaryKey &&
        !join.jsonql;
    /** Manual toggle to advanced mode */
    const [forceAdvanced, setForceAdvanced] = (0, react_1.useState)(false);
    /** Manual toggle for JsonQL mode (very advanced joins) */
    const [forceJsonQLMode, setForceJsonQLMode] = (0, react_1.useState)(false);
    const handleReset = () => {
    ...
    if (forceAdvanced || !fromTable || (join && !isStandard1toN)) {
        return (react_2.default.createElement("div", null,
            react_2.default.createElement("button", { type: "button", className: "btn btn-sm btn-link", style: { float: "right" }, onClick: handleReset }, "Reset"),
            ...
            jsonQLMode ? (react_2.default.createElement("div", null,
                react_2.default.createElement("button", { type: "button", className: "btn btn-sm btn-link", style: { float: "right" }, onClick: () => {
                        setForceJsonQLMode(false);
                        onChange(lodash_1.default.omit(partialJoin, "jsonql"));
                    } }, "Normal Mode"),
                react_2.default.createElement(JsonQLEditor, { jsonql: partialJoin.jsonql, onChange: (jsonql) => {
                        onChange(lodash_1.default.extend({}, partialJoin, { jsonql: jsonql }));
                    } }))) : (react_2.default.createElement("div", null,
                react_2.default.createElement("button", { type: "button", className: "btn btn-sm btn-link", style: { float: "right" }, onClick: () => setForceJsonQLMode(true) }, "JsonQL Mode"),
            ...
            join && join.type == "1-n" ? (react_2.default.createElement(bootstrap_1.FormGroup, { key: "inverse", label: "Inverse. Column (schema, not physical) in 'To Table' that is the reverse of this join. Optional" },
                react_2.default.createElement(bootstrap_1.TextInput, { value: partialJoin.inverse || "", onChange: (inverse) => onChange(lodash_1.default.extend({}, partialJoin, { inverse: inverse })), emptyNull: true }))) : null));
    }
    // --- Simple mode
    ...
    return (react_2.default.createElement("div", null,
        react_2.default.createElement("button", { type: "button", className: "btn btn-sm btn-link", style: { float: "right" }, onClick: () => setForceAdvanced(true) }, "Advanced"),
...
exports.JoinEditorComponent = JoinEditorComponent;
