...
module.exports = OmniBoxExprComponent = (function(superClass) {
  ...
  OmniBoxExprComponent.prototype.renderFormulaDropdown = function() {
    var dropdown, escapeRegex, exprUtils, filter, i, len, noTree, opItem, opItems, specials, tree, treeBuilder;
    dropdown = [];
    specials = [];
    ...
    exprUtils = new ExprUtils(this.props.schema);
    opItems = _.where(exprUtils.findMatchingOpItems({
      resultTypes: this.props.types
    }), {
      prefix: true
    });
    for (i = 0, len = opItems.length; i < len; i++) {
      opItem = opItems[i];
      specials.push(H.a({
        key: opItem.op,
