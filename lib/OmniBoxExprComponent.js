...
module.exports = OmniBoxExprComponent = (function(superClass) {
  ...
  OmniBoxExprComponent.prototype.renderModeSwitcher = function() {
    ...
    if (this.state.mode === "formula") {
      if (this.props.types[0] === "number") {
        label = "123";
      } else if ((ref = this.props.types[0]) === "text" || ref === "enum" || ref === "enumset" || ref === "date" || ref === "datetime") {
        label = "abc";
      } else {
        return;
      }
  ...
  OmniBoxExprComponent.prototype.handleDateSelected = function(event) {
    this.setState({
      inputText: event.date.format("l")
    });
    this.refs.input.blur();
    return this.handleBlur();
  };

  OmniBoxExprComponent.prototype.handleDateTimeSelected = function(event) {
    this.setState({
      inputText: event.date.format("lll")
    });
    this.refs.input.blur();
    return this.handleBlur();
  };
  ...
  OmniBoxExprComponent.prototype.renderFormulaDropdown = function() {
    ...
    noTree = this.props.enumValues && (_.isEqual(this.props.types, ["enum"]) || _.isEqual(this.props.types, ["enumset"]));
    if (!noTree) {
      escapeRegex = function(s) {
        return s.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
      };
