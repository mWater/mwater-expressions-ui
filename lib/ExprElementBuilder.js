var EnumSetComponent, ExprElementBuilder, ExprUtils, H, IdLiteralComponent, LinkComponent, OmniBoxExprComponent, R, React, ScoreExprComponent, StackedComponent, TextArrayComponent, WrappedLinkComponent, _,
...
module.exports = ExprElementBuilder = (function() {
  ...
  ExprElementBuilder.prototype.build = function(expr, table, onChange, options) {
    var anyTypeAllowed, booleanOnly, createWrapOp, elem, exprType, idTable, links;
    if (options == null) {
      options = {};
    }
    _.defaults(options, {
    ...
    anyTypeAllowed = !options.types || (indexOf.call(options.types, "boolean") >= 0 && indexOf.call(options.aggrStatuses, "individual") >= 0) || (indexOf.call(options.types, "number") >= 0 && indexOf.call(options.aggrStatuses, "aggregate") >= 0);
    ...
    if ((expr && expr.type === "literal") || (!expr && options.preferLiteral)) {
      ...
      if (exprType === "id" || _.isEqual(options.types, ["id"]) && options.idTable) {
        idTable = options.idTable || this.exprUtils.getExprIdTable(expr);
    ...
    if (!expr || !expr.type || expr.type === "literal") {
      ...
      elem = this.buildField(expr, onChange, {
        key: options.key
      });
  ...
  ExprElementBuilder.prototype.buildScalar = function(expr, onChange, options) {
    var destTable, innerAggrStatuses, innerElem, innerOnChange, j, join, joinCol, joinsStr, len, multipleJoins, ref, ref1, summary;
    if (options == null) {
      options = {};
    }
  ...
  ExprElementBuilder.prototype.buildOp = function(expr, table, onChange, options) {
    var aggr, innerAggrStatuses, items, lhsElem, lhsOnChange, opElem, opItem, opItems, rhs1OnChange, rhs2OnChange, rhsElem, rhsOnChange;
    if (options == null) {
      options = {};
    }
    switch (expr.op) {
      ...
      default:
        ...
        opElem = R(LinkComponent, {
          dropdownItems: opItems.length > 0 ? _.map(opItems, function(oi) {
            return {
              id: oi.op,
              name: oi.name
            };
