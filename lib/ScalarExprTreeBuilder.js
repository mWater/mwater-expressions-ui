        ...
        if (item.type === "section") {
          if (!item.deprecated) {
            ...
            if (!options.filter) {
              ...
              return nodes.push(node);
            } else if (options.depth < 2 && node.children().length > 0) {
              return nodes.push(node);
            }
          }
        } else {
    ...
    if (column.type === "join") {
      ...
      node.children = (function(_this) {
        return function() {
          ...
          includeCount = _this.exprUtils.isMultipleJoins(options.startTable, joins);
          if (!matches) {
            filter = options.filter;
          } else {
            filter = null;
      ...
      if (options.types) {
        if (this.exprUtils.isMultipleJoins(options.startTable, options.joins)) {
          ...
          if (ref2 = this.exprUtils.getExprType({
            type: "field",
            table: options.table,
            column: column.id
          }), indexOf.call(options.types, ref2) < 0) {
            return;
          }
        }
      }
