...
module.exports = ScalarExprTreeBuilder = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(ScalarExprTreeBuilder, [{
    ...
    value: function createTableChildNodes(options) {
      ...
      ref = this.schema.getVariables(); // Include variables
      ...
      if (!options.includeAggr && (!options.idTable || options.idTable === options.table) && (!options.types || indexOf.call(options.types, "id") >= 0)) {
        node = {
    ...
    value: function createColumnNode(options) {
      ...
      var column, fieldExpr, initVal, joins, matches, node, ref, ref1, ref2, ref3, ref4, types;
      ...
      matches = filterMatches(options.filter, node.name) || node.desc && filterMatches(options.filter, node.desc); // If join (or id, id[]), add children

      if ((ref = column.type) === "join" || ref === "id" || ref === "id[]") {
        ...
        if (column.type === "id" && (!options.types || indexOf.call(options.types, 'id') >= 0) && (!options.idTable || column.idTable === options.idTable)) {
          node.value = {
            table: options.startTable,
            joins: options.joins,
            expr: {
        ...
        if (column.type === "join" && ((ref1 = column.join.type) === 'n-1' || ref1 === '1-1') && (!options.types || indexOf.call(options.types, 'id') >= 0) && (!options.idTable || column.join.toTable === options.idTable)) {
          node.value = {
            table: options.startTable,
            joins: options.joins,
            expr: {
        ...
        if (column.type === "join" && ((ref2 = column.join.type) === 'n-n' || ref2 === '1-n') && (!options.types || indexOf.call(options.types, 'id[]') >= 0) && (!options.idTable || column.join.toTable === options.idTable)) {
          node.value = {
            table: options.startTable,
            joins: options.joins,
            expr: {
        ...
        if (!this.schema.getTable(options.table).ordering && this.exprUtils.isMultipleJoins(options.startTable, options.joins) && (ref3 = column.type) !== 'number' && ref3 !== 'text') {
          return;
        }
        ...
        if (options.types) {
          ...
          if (this.exprUtils.isMultipleJoins(options.startTable, options.joins)) {
            ...
            if (ref4 = this.exprUtils.getExprType(fieldExpr), indexOf.call(options.types, ref4) < 0) {
              return;
            }
          }
        }
