...
module.exports = ScalarExprTreeBuilder = (function() {
  ...
  ScalarExprTreeBuilder.prototype.createTableChildNodes = function(options) {
    ...
    if (!options.includeAggr && options.idTable === options.table && (!options.types || indexOf.call(options.types, "id") >= 0)) {
      ...
      if (!options.filter || options.filter(node.name)) {
        nodes.push(node);
      }
    }
    ...
    if (options.includeAggr && options.depth > 0 && (!options.filter || options.filter("Advanced"))) {
      nodes.push({
        name: "Advanced...",
      ...
      return function(item) {
        ...
        if (item.type === "section") {
          if (!item.deprecated) {
            ...
            matches = !options.filter || options.filter(name) || options.filter(desc);
  ...
  ScalarExprTreeBuilder.prototype.createColumnNode = function(options) {
    ...
    matches = !options.filter || options.filter(node.name) || options.filter(node.desc);
    if (column.type === "join") {
