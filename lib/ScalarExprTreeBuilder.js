...
module.exports = ScalarExprTreeBuilder = class ScalarExprTreeBuilder {
      ...
      ((item) => {
        ...
        if (item.type === "section") {
          ...
          if (!item.deprecated) {
            ...
            name = ExprUtils.localizeString(item.name, this.locale) || "(unnamed)";
            desc = ExprUtils.localizeString(item.desc, this.locale);
            matches = filterMatches(options.filter, name) || (desc && filterMatches(options.filter, desc));
            // Override matching
            overrideMatch = typeof this.isScalarExprTreeSectionMatch === "function" ? this.isScalarExprTreeSectionMatch({
              tableId: options.table,
              section: item,
              filter: options.filter
            }) : void 0;
            ...
            if (typeof this.isScalarExprTreeSectionInitiallyOpen === "function" ? this.isScalarExprTreeSectionInitiallyOpen({
              tableId: options.table,
              section: item,
              filter: options.filter
            }) : void 0) {
  ...
  createColumnNode(options) {
    ...
    if (column.type === "join") {
      ...
      node.children = () => {
        ...
        includeAggr = this.exprUtils.isMultipleJoins(options.startTable, joins);
