    ...
    render() {
        ...
        return R("div", { className: "input-group" }, this.props.multiline
            ...
            : R("input", {
                ...
                value: currentText,
                placeholder
            }), R("div", { className: "input-group-btn" }, R("button", { type: "button", className: "btn btn-default dropdown-toggle", "data-toggle": "dropdown" }, this.state.selectedLanguageCode, " ", R("span", { className: "caret" })), R("ul", { className: "dropdown-menu" }, lodash_1.default.map(availableLanguages, (availableLanguage) => {
            return R("li", { key: availableLanguage.id }, R("a", { onClick: this.onLanguageSelectionClick.bind(null, availableLanguage.id) }, availableLanguage.name));
        }))));
    }
}
