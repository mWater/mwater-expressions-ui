...
export const JoinEditorComponent = (props: {
  ...
  const inverse =
    join && join.type == "1-n" && join.toTable && join.inverse ? schema.getColumn(join.toTable, join.inverse) : null
  const fromTable = schema.getTable(fromTableId)
  const isStandard1toN =
    join &&
    inverse &&
    fromTable &&
    inverse.type == "id" &&
        ...
        {jsonQLMode ? (
          <div>
            ...
            <JsonQLEditor
              ...
              onChange={(jsonql) => {
                onChange(_.extend({}, partialJoin, { jsonql: jsonql }))
              }}
            />
          </div>
        ) : (
...
function JsonQLEditor(props: { jsonql: any; onChange: (jsonql: any) => void }) {
  ...
  return (
    <FormGroup label="JsonQL expression for join. Use {to} and {from} as table aliases">
      <textarea
        rows={10}
