...
export default ScalarExprTreeBuilder = class ScalarExprTreeBuilder {
  ...
  createTableChildNodes(options) {
    ...
    if (!options.includeAggr && (!options.idTable || (options.idTable === options.table)) && (!options.types || options.types.includes("id"))) {
      node = {
      ...
      (item => {
        ...
        if (item.type === "section") {
          ...
          if (!item.deprecated) {
            ...
            // Override matching
            const overrideMatch = this.isScalarExprTreeSectionMatch?.({ tableId: options.table, section: item, filter: options.filter });
            ...
            if (this.isScalarExprTreeSectionInitiallyOpen?.({ tableId: options.table, section: item, filter: options.filter })) {
  ...
  createColumnNode(options) {
    ...
    if (["join", "id", "id[]"].includes(column.type)) {
      ...
      // Handle id
      if ((column.type === "id") && (!options.types || options.types.includes('id')) && (!options.idTable || (column.idTable === options.idTable))) {
      ...
      if ((column.type === "join") && ['n-1', '1-1'].includes(column.join.type) && (!options.types || options.types.includes('id')) && (!options.idTable || (column.join.toTable === options.idTable))) {
        node.value = { table: options.startTable, joins: options.joins, expr: { type: "field", table: options.table, column: column.id } };
      }
      // Multiple joins have a value of id[] (if for correct table)
      if ((column.type === "join") && ['n-n', '1-n'].includes(column.join.type) && (!options.types || options.types.includes('id[]')) && (!options.idTable || (column.join.toTable === options.idTable))) {
        node.value = { table: options.startTable, joins: options.joins, expr: { type: "field", table: options.table, column: column.id } };
      }
      ...
      if (!this.schema.getTable(options.table).ordering && this.exprUtils.isMultipleJoins(options.startTable, options.joins) && !['number', 'text'].includes(column.type)) {
        return;
      }
